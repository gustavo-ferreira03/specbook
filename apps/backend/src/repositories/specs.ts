...
export async function listSpecs(projectId: string): Promise<Spec[]> {
    return db
        .select()
        .from(specs)
...
export async function updateSpecStatusForVersion(
    id: string,
    currentVersionId: string,
    status: SpecStatus,
): Promise<void> {
...
export async function publishSpecVersion(
    id: string,
    currentVersionId: string,
...
export async function deleteSpecRecord(id: string): Promise<void> {
    await db.delete(specs).where(eq(specs.id, id));
}

export async function deleteSpecWithRelations(id: string): Promise<DeleteSpecResult> {
...
export async function addSpecVersion(input: {
    specId: string;
    version: number;
    humanSpec: HumanSpec;
...
export async function getSpecVersion(id: string): Promise<SpecVersion | null> {
    const rows = await db.select().from(specVersions).where(eq(specVersions.id, id));
    return rows[0] ?? null;
}

export async function deleteSpecVersion(id: string): Promise<void> {
    await db.delete(specVersions).where(eq(specVersions.id, id));
}

export async function latestVersionNumber(specId: string): Promise<number> {
    const rows = await db
        .select()
        .from(specVersions)
